...
class GratisDNS
{
    ...
    function deleteSecDomain($domain) {
        $html = $this->_request(array(
            'action' => 'deletesecondarydns',
            'user_domain' => $domain
        ));
    ...
    function createDomain($domain, $type = 'primary', $primary_ns = false, $secondary_ns = false) {
        if ($type == 'primary') {
            $post_array = array(
                'action' => 'createprimaryandsecondarydnsforthisdomain',
                'user_domain' => $domain
            );
        }
        else {
